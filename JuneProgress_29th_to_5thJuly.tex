...
To obtain the corresponding values for $\bbC^t$, we perform the simple operation shown below
 ...
 $$ c^{t}_l=3c^{tt'}_{m}+t'$$ where $$ m=\Big \lfloor \frac{l}{\tau} \Big \rfloor, ~t'=\bmod(l,\tau),t=0,1,...,\tau-1,~l=0,1,...,L-1$$
 
 The above process introduces some chaos into $\bbC^t$ which will be useful in dealing with weight-$4$ RTZ inputs.
